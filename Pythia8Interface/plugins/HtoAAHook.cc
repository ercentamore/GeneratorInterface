...
static double lookup_invpdf2d(double m1, double m2,
                              const std::vector<double> &mBins,
                              const std::vector<double> &invpdf2d) {
    const size_t n = mBins.size();
    size_t ib1 = find_bin(m1, mBins);
    size_t ib2 = find_bin(m2, mBins);
    return invpdf2d[ib1 * n + ib2];
}

static std::vector<int> load_or_init_csv2d(const std::string &filename,
                                           size_t n1, size_t n2) {
  std::vector<int> occ(n1 * n2, 100); // flat default
  ...
  size_t idx = 0;
  while (idx < occ.size() && std::getline(ifs, line)) {
...
public:
    explicit HtoAAhook(const edm::ParameterSet &ps)
            ...
            auto occ2d = load_or_init_csv2d(csv_, bins_.size(), bins_.size());
    ...
    bool doVetoProcessLevel(Event &event) override {
        ...
        for (int iH = 0; iH < event.size(); ++iH) {
            ...
            do {
                m1 = rndmPtr->flat() * (maxM_ - minM_) + minM_;
                m2 = rndmPtr->flat() * (maxM_ - minM_) + minM_;
                if (m1 + m2 >= MM) continue; // kinematic veto
                weight = lookup_invpdf2d(m1, m2, bins_, invpdf2d_);
                u = rndmPtr->flat();
            } while (u > weight);
...
private:
    ...
    // Pre-computed tables.
    std::vector<double> bins_, invpdf2d_;
